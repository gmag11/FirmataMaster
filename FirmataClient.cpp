...
*/
void FirmataClientClass::setVersion(version * ver, int major, int minor) {
...
*/
void FirmataClientClass::setFirmwareName(String name) {
	firmwareName = name;
#ifdef DEBUG_FIRMATA
...
#ifdef DEBUG_FIRMATA
				DBG_PORT.printf("REPORT FIRMATA PROTOCOL VERSION: %d.%d\r\n", storedInputData[1], storedInputData[0]);
#endif // DEBUG_FIRMATA
				setVersion(&firmataProtocolVersion, storedInputData[1], storedInputData[0]);
				break;
...
void FirmataClientClass::processSysexMessage() {
...
#ifdef DEBUG_SYSEX
	DBG_PORT.printf("Process SysEx: SE Command: %X Bytes read: %d\r\n", SyXCommand, sysexBytesRead);
#endif // DEBUG_SYSEX
	...
	case REPORT_FIRMWARE:
...
#endif // DEBUG_SYSEX
		String temp = "";
		setVersion(&firmataFirmwareVersion, storedInputData[1], storedInputData[2]); // Store Firmware version
#ifdef DEBUG_FIRMATA
		DBG_PORT.printf("REPORT FIRMATA FIRMWARE VERSION: %d.%d\r\n", storedInputData[1], storedInputData[2]);
#endif //DEBUG_FIRMATA
		...
		setFirmwareName(temp); // Store firmware name
		break;
	}
}
